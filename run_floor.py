...
#
# The script reproduces the numerical example with the texture 'Floor' in the paper.
...
def Trainset(size=1000):
	...
	'''
	t = []
	picts = os.listdir('training_img/lr_floor')
	for img in picts:
		lr = wp.imread('training_img/lr_floor/' + img)
		t.append(lr)
...
def Validationset():
	...
	'''
	val = []
	picts_hr = os.listdir('training_img/validation_floor/val_hr')
	picts_lr = os.listdir('training_img/validation_floor/val_lr')
	for i in range(len(picts_hr)):
		val_hr = wp.imread('training_img/validation_floor/val_hr/' + picts_hr[i])
		val_lr = wp.imread('training_img/validation_floor/val_lr/' + picts_lr[i])
...
reference_img = wp.imread('test_img/ref_floor.png')
real_test = wp.imread('test_img/hr_floor.png')
test_img = wp.imread('test_img/lr_floor.png')# = Downsample(real_test) + 0.01*torch.randn_like(Downsample(real_test))
...
def training(trainset, model, reference_img, batch_size, epochs, args = args, opti = OPTIMIZER):
    ...
    for t in range(epochs):
        ...
        if (t+1)%val_step == 0:
            ...
            plt.title(title)
            plt.savefig('checkpoints/ValidatonPSNR_floor.png')
            plt.close()
            print(f'-------------------------------')
        
        #save a checkpoint
        if (t+1)%30 == 0:
            torch.save({'net_state_dict': model.state_dict(), 'optimizer_state_dict': opti.state_dict()}, 'checkpoints/epoch' + str(t+1) + 'floor.pth')
            ...
            plt.yscale('log')
            plt.savefig('checkpoints/losscurve_floor.png')
            plt.close()
...
if __name__ == '__main__':
    if not os.path.isdir('results'):
       os.mkdir('results')    
    if retrain:
        training(t,net,reference_img,25,270)
        #torch.save({'net_state_dict': net.state_dict(), 'optimizer_state_dict': OPTIMIZER.state_dict()}, 'results/weights_floor.pth')        
        pred = net(test_img)
        save_img(pred,'results/pred_w2_floor')
    if not retrain:
        weights = torch.load('results/weights_floor.pth')
