...
class Wasserstein_trainer(nn.Module):
    def __init__(self,net,optimizer,operator,trainset,valset,
        ...
        self.keops = keops
        self.center = center
        self.data_fid = nn.MSELoss(reduction='sum')
    ...
    def train(self,batch_size,epochs,savename):
        ...
        for ep in range(epochs):
            ...
            for obs in loop:
                ...
                z = torch.randn([obs.shape[0],1,args.hr_size,args.hr_size],
                                device=DEVICE)
                pred, logdet = self.net(z, obs)
                pred_pat = self.im2patch(pred)
                down_pred = self.operator(pred)
                
                wloss = self.WLoss(pred_pat)
                l2 = self.data_fid(down_pred,obs) / obs.shape[0] #||f(T(z,y)) - y||^2
                loss = l2/2 + self.lam * wloss - (args.std_noise**2)*torch.mean(logdet)
            ...
            with torch.no_grad():
                ...
                for gt,obs in val_loader:
                    gt = gt.to(DEVICE)
                    preds = torch.empty(0,1,600,600,device=DEVICE)
                    with torch.no_grad():
                        for _ in range(10):
                            z = torch.randn_like(gt)
                            pred, _ = self.net(z, obs.to(DEVICE))
                            preds = torch.cat([preds,pred])
                    mean = torch.mean(preds,dim=0)
                    mse = self.data_fid(mean,gt)
...
if __name__ == '__main__':
    ...
    train_group.add_argument('-lr', '--learning_rate', type=float, default=2e-4)
    ...
    wloss_group.add_argument('-l', '--lam', type=float, default=100,
    ...
    if not os.path.isdir('results'):
       os.mkdir('results')    
    args.hr_size = 100 if args.scale == 4 else 160
    net = flow.WPPFlow(scale=args.scale,hr_size=args.hr_size).to(device=DEVICE)
    
    print(f'Superresolution WPPFlow with magnification factor {args.scale}')
    
    if args.train:
        operator = utils.SR_operator(scale_factor=1/args.scale, 
                            gaussian_std = int(args.scale/2))

        opti = torch.optim.Adam(net.parameters(), lr=args.learning_rate)    
        
        lr_train = utils.SiC(datafile = 
                    f'{base}training_img/train_{args.imageclass}_x{args.scale}.h5',
                    split = 'train')
        val = utils.SiC(datafile = 
                    f'{base}training_img/val_{args.imageclass}_x{args.scale}.h5',
                    split = 'val')
        ...
        savename = f'WPPFlow_x{args.scale}_exp{args.num}'
        ...
        torch.save({'net_state_dict': wtrainer.net.state_dict(), 
                    ...
                    f'{base}results/weights_WPPFlow_x{args.scale}.pth')

    else:
        with torch.no_grad():
            hr = utils.imread(f'test_img/hr_{args.imageclass}.png')
            lr = utils.imread(f'test_img/lr_{args.imageclass}_x{args.scale}.png')
            #  = operator(hr) + 0.01*torch.randn_like(operator(hr))
            name = f'{base}results/weights_WPPFlow_x{args.scale}.pth'
            ...
            io.imsave(f'results/W2_WPPFlow_x{args.scale}.png',
                        np.clip(mean.squeeze().numpy(),0,1))
