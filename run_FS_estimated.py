...
#
# The script reproduces the numerical example with the material 'FS estimated forward operator'
# in the paper.
...
def WLoss(args, input_img, ref_pat, model, psi):
    ...
    down  = torch.empty(0,device=DEVICE)
    for i in range(pred.shape[0]):
        tmp = operator(pred[i,:,:,:].unsqueeze(0)) #apply operator for each image separately
        down = torch.cat([down,tmp])
    down_pred = down #downsample image by factor 2
...
def training(trainset, model, reference_img, batch_size, epochs, args, opti):
    ...
    for t in tqdm(range(epochs)):
        ...
        if (t+1)%val_step == 0:
            ...
            plt.title(title)
            plt.savefig('checkpoints/ValidatonPSNR_FS_registered.pdf')
            plt.close()
            print(f'-------------------------------')
        
        #save a checkpoint
        if (t+1)%30 == 0:
            torch.save({'net_state_dict': model.state_dict()}, 'checkpoints/checkpoint_FS_registered.pth')
            with torch.no_grad():
                pred_hr = model(lr)
            ...
            plt.yscale('log')
            plt.savefig('checkpoints/losscurve_FS_registered.pdf')
            plt.close()
...
if __name__ == '__main__':
    ...
    print('Superresolution for the material FS with estimated operator')
    
    scale_factor = 2
    net = model.small_acnet.Net(scale=scale_factor).to(device=DEVICE)

    hr = utils.imread('test_img/hr_FS_registered.png')
    lr = utils.imread('test_img/lr_FS_registered.png')

    if retrain:
        #inputs
        image_class = 'FS_registered'
        
        #load estimated forward operator
        operator = SR_operator(0.5).to(DEVICE)
        ...
        reference_img = utils.imread('test_img/ref_FS_registered.png')        
        ...
        torch.save({'net_state_dict': net.state_dict(), 'optimizer_state_dict': OPTIMIZER.state_dict()},
                    'results/weights_FS_registered.pth')        
        utils.save_img(pred,'results/W2_FS_registered')
            
    if not retrain:
        weights = torch.load('results/weights_FS_registered.pth')
